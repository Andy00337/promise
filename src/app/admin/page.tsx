...
export default function AdminPage() {
  ...
  const playAnimation = () => {
    ...
    const interval = setInterval(() => {
      ...
      const marker = new TMap.MultiMarker({
        ...
        styles: {
          default: new TMap.MarkerStyle({ width: 20, height: 20, color: '#FF6B6B' }),
        },
        geometries: [{
          id: String(i),
          position: latlng,
          properties: { time: formatDate(s.scannedAt) },
        }],
      });
      ...
      if (path.length > 1) {
        ...
        polylineRef.current = new TMap.MultiPolyline({
          map: mapInstance.current,
          styles: {
            line: new TMap.PolylineStyle({ color: '#FF6B6B', width: 4 }),
          },
  ...
  if (loading) return <div className="p-10">加载数据中...</div>;

  return (
    <main className="p-6 max-w-6xl mx-auto">
      <h1 className="text-2xl font-bold mb-4">扫码轨迹后台</h1>
      <p className="text-sm text-gray-500 mb-4">
        共 {scans.length} 条有效定位记录
      </p>
      ...
      <div className="flex gap-3 mb-4">
        <button
          ...
          disabled={isPlaying}
          className="rounded-lg bg-rose-500 px-4 py-2 text-white disabled:opacity-50"
        >
